...
stdenv.mkDerivation {
  ...
  postInstall = ''
    wrapProgram $out/bin/de.capypara.FieldMonitor.Devel --prefix PATH ':' "$out/libexec" --set RUST_LOG 'field_monitor=trace,libfieldmonitor=trace,oo7=debug,rdw=debug,rdw-vnc=debug,rdw-spice=debug,rdw-rdp=debug,vte=debug,gtk-vnc=debug,usbredirhost=debug,Spice=debug,spice_gtk_usb_portal=debug,Adwaita=info,GLib=info,warning' --set RUST_BACKTRACE '1'
  '';
