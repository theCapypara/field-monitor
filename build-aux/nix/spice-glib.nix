# See https://github.com/NixOS/nixpkgs/pull/550821
# ------------------------------------------------
# NOTE: This package is built without GTK 3 support by default.
# It can provides the package `spice-gtk` instead (withGtk=true; see `spice-gtk` in all-packages.nix).
{
  lib,
  stdenv,
  fetchFromGitLab,
...
stdenv.mkDerivation (finalAttrs: {
  ...
  # Up until v0.42 the package was distributed via https://www.spice-space.org/download/gtk/,
  # however 0.43 was not published there.
  src = fetchFromGitLab {
  ...
  postUnpack = ''
    echo "0.43" > source/.tarball-version
  '';
  ...
  meta =
    let
      descriptions =
        if withGtk then
          {
            description = "SPICE Glib client library with GTK+3 widgets";
            longDescription = ''
          ...
          {
            description = "SPICE Glib client library";
            longDescription = ''
              spice-glib provides support for Glib apps to interact with the SPICE
    ...
    in
    {
      inherit (descriptions) description longDescription;
